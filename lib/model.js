...
var Model = module.exports = function (rawElements, metadata) {
	...
	this.bucket = this.rodm.db.bucket(this.schema.name).ofType('searchable');
...
Model.prototype.updateProps = function() {
	var self = this;
...
Model.prototype.insert = Model.prototype.update = function(cb, opts) {
	...
	if(this.check()) {
		...
		this.bucket.ofType('searchable').objects.save(obj, function(err, result) {
			if(err)
				return cb(err);
...
Model.findById = function(id, cb) {
	...
	var self = this;
	this.bucket.ofType('searchable').objects.get(id, function(err, obj) {
		if(err)	cb(err);
		obj.data['id'] = obj.key;
...
Model.exists = function(id, cb) {
	...
	this.bucket.ofType('searchable').object.exists(id, cb);
};
